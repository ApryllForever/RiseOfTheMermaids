...
{
	[XmlType("Mods_ApryllForever_RestStopLocations_BluebellaDungeon")]
	public class BluebellaDungeon : HellLocation
	{
		...
		public static List<BluebellaDungeon> activeLevels = new List<BluebellaDungeon>();
		...
		protected static Point bluebellaShortcutOutPosition = new Point(29, 34);

		[XmlIgnore]
		protected NetBool bluebellaShortcutOutUnlocked = new NetBool(value: false);
		...
		[XmlIgnore]
		public NetList<ElfGate, NetRef<ElfGate>> elfGates = new NetList<ElfGate, NetRef<ElfGate>>();

		private int lavaSoundsPlayedThisTick;
		
		public const string BaseLocationName = "BluebellaDungeon";

		public const string LocationRoomInfix = "Room";

		public const string LocationCaveInfix = "Cave";

		public const int BossLevel = 12;
		...
		public static BluebellaDungeon GetLevelInstance(string locName)
		{
			{
				...
				BluebellaDungeon newLevel = new BluebellaDungeon(t, l, locName);
		...
		public BluebellaDungeon()
		{
			mapContent = Game1.game1.xTileContent.CreateTemporary();
			mapPath.Value = "Maps\\Mines\\VolcanoTemplate"; // (Mod.instance.Helper.ModContent.GetInternalAssetName("assets/maps/HellDungeonBelt.tmx").BaseName); 
		}

		public BluebellaDungeon(LevelType type, int level, string name)
			: this()
		{
			levelType.Value = type;
			this.level.Value = level;
			this.name.Value = "BluebellaDungeon" + level;
		}

        public BluebellaDungeon(int l, string locName)
        {
            this.l = l;
            LocName = locName;
        }
		...
		{
			base.initNetFields();
			base.NetFields.AddField(this.level).AddField(this.coolLavaEvent).AddField(this.cooledLavaTiles.NetFields).AddField(this.generationSeed).AddField(this.layoutIndex).AddField(this.elfGates).AddField(this.bluebellaShortcutOutUnlocked);
			coolLavaEvent.onEvent += OnCoolLavaEvent;
			bluebellaShortcutOutUnlocked.InterpolationWait = false;
			bluebellaShortcutOutUnlocked.fieldChangeEvent += delegate (NetBool f, bool oldValue, bool newValue)
			{
				if (newValue && mapPath.Value != null)
				{
					UpdatebluebellaShortcutOut();
				}
			};
		}
		...
		public void UseBluebellaShortcut()
		{
			DelayedAction.playSoundAfterDelay("fallDown", 200);
			DelayedAction.playSoundAfterDelay("clubSmash", 900);
			Game1.player.CanMove = false;
			Game1.player.jump();
			Game1.warpFarmer("Custom_SapphireSprings", 12, 5, 1);
		}

		public virtual void Generate( bool use_level_level_as_layout = false)
		{
			generated = true;
			if (Game1.IsMasterGame)
			{
				...
				else
				{
					...
					if ((int)level.Value > 1 && layout_random.NextDouble() < 0.5 * (double)luckMultiplier)
					{
			...
			if (level.Value == 4)
			{
				...
				var elfMap = Game1.game1.xTileContent.Load<Map>(Mod.instance.Helper.ModContent.GetInternalAssetName("Assets/Maps/ElfShop.tmx").BaseName);

				ApplyMapOverride(elfMap, "ElfShop", (Microsoft.Xna.Framework.Rectangle?)null, (Microsoft.Xna.Framework.Rectangle?)new Microsoft.Xna.Framework.Rectangle(34, 29, 5, 4));
		...
		{
			if ((int)layoutIndex.Value >= 32)
			{
				return (int)layoutIndex.Value <= 34;
			}
			return false;
		}
		...
		{
			if ((int)layoutIndex.Value >= 35)
			{
				return (int)layoutIndex.Value <= 37;
			}
			return false;
		}
		...
		public virtual void UpdatebluebellaShortcutOut()
		{
			if (this == Game1.currentLocation)
			{
				if (bluebellaShortcutOutUnlocked.Value)
				{
					setMapTileIndex(bluebellaShortcutOutPosition.X, bluebellaShortcutOutPosition.Y, 367, "Buildings");
					removeTile(bluebellaShortcutOutPosition.X, bluebellaShortcutOutPosition.Y - 1, "Front");
				}
				else
				{
					setMapTileIndex(bluebellaShortcutOutPosition.X, bluebellaShortcutOutPosition.Y, 399, "Buildings");
					setMapTileIndex(bluebellaShortcutOutPosition.X, bluebellaShortcutOutPosition.Y - 1, 383, "Front");
				}
			}
		}
		...
		protected override void resetLocalState()
		{
			...
			if (level.Value == 4 || level.Value == 8)
			{
				UpdatebluebellaShortcutOut();
			}
			...
			if (level.Value == 0 && Game1.player.previousLocationName == "Custom_MoranaHome")
			{
			...
			foreach (ElfGate elfGate in elfGates)
			{
				elfGate.ResetLocalState();
			}
			if ((int)level.Value == 4 || (int)level.Value == 8)
			{
				Game1.changeMusicTrack("Volcano_Ambient");
				AmbientLocationSounds.addSound(new Vector2(29f, 31f), 0);
			}
			else if ((int)level.Value == 1 || ((Game1.random.NextDouble() < 0.25 || (int)level.Value == 5) && (Game1.getMusicTrackName() == "none" || Game1.isMusicContextActiveButNotPlaying() || Game1.getMusicTrackName().EndsWith("_Ambient"))))
			{
			...
			ApplyMapOverride("Mines\\Volcano_Well", (Microsoft.Xna.Framework.Rectangle?)null, (Microsoft.Xna.Framework.Rectangle?)new Microsoft.Xna.Framework.Rectangle(22, 50, 6, 4));
			for (int x = 24; x < 28; x++)
			{
				for (int y = 50; y < 54; y++)
				{
		...
		{
			if ((int)level.Value == 4 && new Microsoft.Xna.Framework.Rectangle(27, 29, 4, 4).Contains(tileX, tileY))
			{
				return true;
			}
			if ((int)level.Value == 8 && new Microsoft.Xna.Framework.Rectangle(27, 29, 4, 4).Contains(tileX, tileY))
			{
				return true;
			}
			if ((int)level.Value == 0 && tileX > 23 && tileX < 28 && tileY > 42 && tileY < 47)
			{
				return true;
			}
			return false;
		...
		public virtual void GenerateLevel(bool use_level_level_as_layout = false)
		{
			...
			Texture2D layout_texture = Game1.temporaryContent.Load<Texture2D>(Mod.instance.Helper.ModContent.GetInternalAssetName("assets/maps/Layouts.png").BaseName);
			mapWidth = 64;
			mapHeight = 64;
            ...
            base.map.TileSheets[0].TileIndexProperties[1].Add("Type", "Stone");
            base.map.TileSheets[0].TileIndexProperties[2].Add("Type", "Stone");
            base.map.TileSheets[0].TileIndexProperties[3].Add("Type", "Stone");
            base.map.TileSheets[0].TileIndexProperties[17].Add("Type", "Stone");
            base.map.TileSheets[0].TileIndexProperties[18].Add("Type", "Stone");
            base.map.TileSheets[0].TileIndexProperties[19].Add("Type", "Stone");
            base.map.TileSheets[0].TileIndexProperties[528].Add("Type", "Stone");
            base.map.TileSheets[0].TileIndexProperties[544].Add("Type", "Stone");
            base.map.TileSheets[0].TileIndexProperties[560].Add("Type", "Stone");
            base.map.TileSheets[0].TileIndexProperties[545].Add("Type", "Stone");
            base.map.TileSheets[0].TileIndexProperties[561].Add("Type", "Stone");
            base.map.TileSheets[0].TileIndexProperties[564].Add("Type", "Stone");
            base.map.TileSheets[0].TileIndexProperties[565].Add("Type", "Stone");
            base.map.TileSheets[0].TileIndexProperties[555].Add("Type", "Stone");
            base.map.TileSheets[0].TileIndexProperties[571].Add("Type", "Stone");
			...
			ApplyPixels("assets/maps/Layouts.png", layout_offset_x, layout_offset_y, mapWidth, mapHeight, 0, 0, flip_x);
			...
			ApplyToColor(new Color(0, 255, 0), delegate (int x, int y)
			{
				...
				if (level.Value == 0)
				{
					warps.Add(new Warp(x, y + 2, "Custom_SapphireSprings", 12, 5, flipFarmer: false));
				}
				else
				{
					warps.Add(new Warp(x, y + 2, "BluebellaDungeon" + (level.Value - 1), x - startPosition.Value.X, 0, flipFarmer: false));
				}
			});
			ApplyToColor(new Color(255, 0, 0), delegate (int x, int y)
			{
				...
				if (level.Value == 11)
				{
					warps.Add(new Warp(x, y - 2, "Custom_MoranaHome", 21, 39, flipFarmer: false));
				}
				else
				{
					warps.Add(new Warp(x, y - 2, "BluebellaDungeon" + (level.Value + 1), x - endPosition.Value.X, 1, flipFarmer: false));
				}
			});
			setPieceAreas.Clear();
			...
			if ((int)level.Value != 0)
			{
				GenerateDirtTiles();
			}
			if ((level.Value == 11 || generationRandom.NextDouble() < (isMonsterLevel() ? 1.0 : 0.2)) && possibleSwitchPositions.ContainsKey(0) && possibleSwitchPositions[0].Count > 0)
			{
			...
			foreach (int index in possibleGatePositions.Keys)
			{
				...
				{
                    Point gate_point;
                    gate_point = this.generationRandom.ChooseFrom(this.possibleGatePositions[index]);
                    CreateElfGate(index, gate_point);
                }
			}
			if ((int)level.Value == 0)
			{
				...
				CreateElfGate(-1, new Point(40, 48));
				...
				warps.Add(new Warp(44, 48, "Custom_MoranaHome", 11, 36, flipFarmer: false));
				//CreateEntrance(new Point(6, 48));
				//warps.Add(new Warp(6, 50, "Custom_SapphireSprings", 12, 5, flipFarmer: false));
			}
			if (Game1.IsMasterGame)
			{
				GenerateEntities();
		...
		public virtual void CreateElfGate(int gate_index, Point tile_position)
		{
			...
			if (Game1.IsMasterGame)
			{
				ElfGate gate = new ElfGate(this, gate_index, tile_position.X, tile_position.Y, seed);
				elfGates.Add(gate);
			}
		}
		...
		{
			if ((int)level.Value == 0 || (int)level.Value == 4 || (int)level.Value == 8)
			{
				monsterChance = 0.05;
				itemChance = 0.1;
				gemStoneChance = 0.0;
		...
		public bool isTileClearForMineObjects(Vector2 v, bool ignoreRuins = false)
		{
            ...
            string s;
            s = this.doesTileHaveProperty((int)v.X, (int)v.Y, "Type", "Back");
            ...
            {
                int tileIndex;
                tileIndex = base.getTileIndexAt((int)v.X, (int)v.Y, "Back");
		...
		public virtual void GenerateEntities()
		{
			...
			double monsterChance = 0.004 + (double)generationRandom.Next(70) / 10000.0;
			...
			if ((int)level.Value > 0 && (int)level.Value != 4 && (int)level.Value != 8 && (generationRandom.NextDouble() < 0.5 || isMushroomLevel()))
			{
				...
				for (int k = 0; k < numBarrels; k++)
				{
					...
					while (isTileOnMap(p2.X, p2.Y))
					{
						...
						if (isTileClearForMineObjects(new Vector2(p2.X, p2.Y)))
						{
							...
							{
                                base.objects.Add(objectPos2, BreakableContainer.GetBarrelForVolcanoDungeon(objectPos2));
                            }
							break;
						}
					}
			...
			if ((int)level.Value != 4 || (int)level.Value != 8)
			{
				for (int x2 = 0; x2 < map.Layers[0].LayerWidth; x2++)
				{
					for (int y2 = 0; y2 < map.Layers[0].LayerHeight; y2++)
					{
						...
						if (isTileOnClearAndSolidGround(objectPos) && generationRandom.NextDouble() < monsterChance)
						{
							...
							{
								characters.Add(new Bat(objectPos * 64f, ((int)level.Value > 4 && generationRandom.NextDouble() < 0.5) ? (-556) : (-555)));
							}
						}
						else
						{
			...
			{
                if (this.CanSpawnCharacterHere(new Vector2(p.X, p.Y)))
                {
					int direction = 1;
        ...
        {
            int whichStone;
            whichStone = this.chooseStoneTypeIndexOnly(tile);
            int stoneHealth;
            stoneHealth = 1;
            switch (whichStone)
            {
        ...
        private int chooseStoneTypeIndexOnly(Vector2 tile)
		{
			...
			float levelMod = 1f + (float)(int)level.Value / 7f;
			...
			else
			{
				...
				{
					if (objects.ContainsKey(v4) && (int)objects[v4].parentSheetIndex.Value == 765)
					{
						chance5 += 0.1;
					}
				}
				...
				else
				{
					...
					{
						if (objects.ContainsKey(v3) && (int)objects[v3].parentSheetIndex.Value == 764)
						{
							chance5 += 0.2;
						}
					}
					...
					else
					{
						...
						{
							if (objects.ContainsKey(v2) && (int)objects[v2].parentSheetIndex.Value == 850)
							{
								chance5 += 0.25;
							}
						}
						...
						else
						{
							...
							{
								if (objects.ContainsKey(v) && (int)objects[v].parentSheetIndex.Value == 849)
								{
									chance5 += 0.25;
								}
							}
			...
			if ((int)level.Value > 2 && generationRandom.NextDouble() < 0.0002)
			{
				whichStone2 = 46;
			}
			return whichStone2;
        ...
        {
            Random chest_random;
            chest_random = Utility.CreateRandom(this.generationRandom.Next());
            float extraRare_luckboost;
            extraRare_luckboost = (float)Game1.player.team.AverageLuckLevel() * 0.035f + (float)Game1.player.team.AverageDailyLuck() / 2f;
            if (Game1.IsMasterGame)
            {
                Vector2 position;
                position = new Vector2(tile_x, tile_y);
                Chest chest;
                chest = new Chest(playerChest: false, position);
                chest.dropContents.Value = true;
                chest.synchronized.Value = true;
                chest.type.Value = "interactive";
                if (chest_random.NextDouble() < (double)(((int)this.level == 9) ? (0.5f + extraRare_luckboost) : (0.1f + extraRare_luckboost)))
                {
                ...
                case 0:
                    {
                        ...
                        switch (random)
                        {
                            ...
                            case 5:
                                ...
                                break;
                            default:
                                items.Add(ItemRegistry.Create("(W)" + chest_random.Next(54, 57)));
                                break;
                        }
                        break;
                    }
                case 1:
                    {
                        ...
                        switch (random2)
                        {
                            ...
                            case 5:
                                ...
                                break;
                            case 6:
                                items.Add(ItemRegistry.Create("(W)" + chest_random.Next(57, 60)));
                                break;
                            case 7:
		...
		{
			if ((int)level.Value != 4 && (int)level.Value != 8 && t is WateringCan && isTileOnMap(new Vector2(tileX, tileY)) && waterTiles[tileX, tileY] && !cooledLavaTiles.ContainsKey(new Vector2(tileX, tileY)))
			{
		...
		public override void UpdateWhenCurrentLocation(GameTime time)
		{
			...
			foreach (ElfGate elfGate in elfGates)
			{
				elfGate.UpdateWhenCurrentLocation(time, this);
		...
		{
			if ((int)level.Value == 4)
			{
				base.drawWaterTile(b, x, y);
				return;
			}
			if ((int)level.Value == 8)
			{
				base.drawWaterTile(b, x, y);
				return;
			}
			if ((int)level.Value == 0 && x > 23 && x < 28 && y > 50 && y < 54)
			{
        ...
        public override void drawAboveAlwaysFrontLayer(SpriteBatch b)
        {
            ...
            if (!Game1.game1.takingMapScreenshot && (int)this.level > 0)
            {
                Color col;
                col = SpriteText.color_Red;
                string txt;
                txt = this.level?.ToString() ?? "";
                Microsoft.Xna.Framework.Rectangle tsarea;
                tsarea = Game1.game1.GraphicsDevice.Viewport.GetTitleSafeArea();
        ...
        public override void performTenMinuteUpdate(int timeOfDay)
		{
			...
			if (Game1.random.NextDouble() < 0.1 && (int)level.Value > 0 && (int)level.Value != 4)
			{
				int numsprites = 0;
		...
		public void spawnFlyingMonsterOffScreen()
		{
			...
			characters.Add(new Bat(spawnLocation, ((int)level.Value > 4 && Game1.random.NextDouble() < 0.5) ? (-556) : (-555))
			{
		...
		public virtual void ApplyPixels(string layout_texture_name, int source_x = 0, int source_y = 0, int width = 64, int height = 64, int x_offset = 0, int y_offset = 0, bool flip_x = false)
		{
			...
			string BluebellaTexture = (Mod.instance.Helper.ModContent.GetInternalAssetName("assets/maps/Layouts.png").BaseName);
			Texture2D texture2D = Game1.temporaryContent.Load<Texture2D>(BluebellaTexture);
		...
		{
			foreach (BluebellaDungeon level in activeLevels)
			{
		...
		public override void cleanupBeforePlayerExit()
		{
			...
			if (Game1.locationRequest != null && !Game1.locationRequest.Name.Contains("Bluebella"))
			{
		...
		{
			activeLevels.RemoveAll(delegate (BluebellaDungeon level)
			{
				level.CleanUp();
		...
		public static void ForEach(Action<BluebellaDungeon> action)
		{
			foreach (BluebellaDungeon level in activeLevels)
			{
				action(level);
		...
		public override bool checkAction(Location tileLocation, xTile.Dimensions.Rectangle viewport, Farmer who)
		{
			...
			if (getTileIndexAt(tileLocation.X, tileLocation.Y, "Buildings") == 77)
			{

				{
					...
					portraitPerson = Game1.getCharacterFromName("Vika");
					...
					Game1.activeClickableMenu = new ShopMenu(stock, 0, "BluebellaElfShop", null, null, "Vika")
					{
						portraitPerson = Game1.getCharacterFromName("Vika")
					};
		...
		public override void performTouchAction(string full_action_string, Vector2 player_standing_position)
		{
			if (!Game1.eventUp)
			{
				...
				if (a == "ElfSwitch")
				{
					...
					foreach (ElfGate gate in elfGates)
					{

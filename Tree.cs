...
namespace RestStopLocations
{
    ...
    public class BigTree : TerrainFeature
	{
		...
		{
			if (BigTree._WildTreeData == null)
			{
				BigTree._LoadWildTreeData();
			}
			return BigTree._WildTreeData;
		}

		/// <summary>Get tree types indexed by their unqualified seed item ID.</summary>
		public static Dictionary<string, string> GetWildTreeSeedLookup()
		{
			if (BigTree._WildTreeSeedLookup == null)
			{
				BigTree._LoadWildTreeData();
			}
			return BigTree._WildTreeSeedLookup;
		}
		...
		{
            BigTree._WildTreeData = Game1.content.Load<Dictionary<string, WildTreeData>>("Data\\WildTrees");
            BigTree._WildTreeSeedLookup = new Dictionary<string, string>();
			foreach (string key in BigTree._WildTreeData.Keys)
			{
				if (!string.IsNullOrEmpty(BigTree._WildTreeData[key].SeedItemId) && BigTree._WildTreeData[key].SeedPlantable)
				{
                    BigTree._WildTreeSeedLookup[BigTree._WildTreeData[key].SeedItemId] = key;
				}
			}
		}
		...
		{
            BigTree._WildTreeData = null;
            BigTree._WildTreeSeedLookup = null;
		}
		...
		protected Item TryGetDrop(WildTreeItemData drop, Random r, Farmer targetFarmer, string fieldName, Func<string, string> formatItemId = null, bool? isStump = null)
		{
			...
			return ItemQueryResolver.TryResolveRandomItem(drop, new ItemQueryContext(this.Location, targetFarmer, r), avoidRepeat: false, null, formatItemId, null, (string query, string error) =>
            {
                IGameLogger log;
                //log = Game1.log;
		...
		public virtual int GetMaxSizeHere(bool ignoreSeason = false)
		{
			...
			if (!ignoreSeason && this.localSeason == Season.Winter && !data.GrowsInWinter && !this.fertilized.Value   )
			{
				return this.growthStage.Value;
			}
		...
		public override void dayUpdate()
		{
			...
			if (this.tapped.Value)
			{
				...
				if (tile_object == null || !tile_object.bigCraftable || tile_object.QualifiedItemId != "(BC)105")
				{
			...
			{
				float chance;
				chance = ((data.GrowthChance >= 0f) ? data.GrowthChance : 0.2f);
				float fertilizedGrowthChance;
				fertilizedGrowthChance = ((data.FertilizedGrowthChance >= 0f) ? data.FertilizedGrowthChance : 1f);
				if (Game1.random.NextBool(chance) || (this.fertilized.Value && Game1.random.NextBool(fertilizedGrowthChance)))
				{
					this.growthStage.Value++;
				}
			}
			if (this.localSeason == Season.Winter && data != null && data.IsStumpDuringWinter)
			{
			...
			{
				float seedChance;
				seedChance = ((data.SeedChance >= 0f) ? data.SeedChance : 0.05f);
				if ((int)this.growthStage >= 5 && Game1.random.NextDouble() < (double)seedChance)
				{
		...
		public override void draw(SpriteBatch spriteBatch)
		{
			...
			if (this.texture.Value == null || !BigTree.TryGetData(this.treeType.Value, out var data))
			{

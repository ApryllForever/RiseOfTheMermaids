...
{
	public class IslandGeodeMenu : MenuWithInventory
	{
		public const int region_geodeSpot = 998;

		public ClickableComponent geodeSpot;

		public AnimatedSprite lilac;
			...
			: base(null, okButton: true, trashCan: true, 12, 132)
		{
			...
			geodeSpot = new ClickableComponent(new Microsoft.Xna.Framework.Rectangle(xPositionOnScreen + IClickableMenu.spaceToClearSideBorder + IClickableMenu.borderWidth / 2, yPositionOnScreen + IClickableMenu.spaceToClearTopBorder + 4, 560, 308), "")
			{
				myID = 998,
				downNeighborID = 0
			};
			lilac = new AnimatedSprite("Characters\\MermaidLilac", 12, 32, 48);
		...
		public virtual void startGeodeCrack()
		{
			...
			lilac.setCurrentAnimation(new List<FarmerSprite.AnimationFrame>
			{
				new FarmerSprite.AnimationFrame(12, 300),
				new FarmerSprite.AnimationFrame(13, 200),
				new FarmerSprite.AnimationFrame(14, 80),
		...
		public override void performHoverAction(int x, int y)
		{
			...
			if (descriptionText.Equals(""))
			{
				...
				{
					descriptionText = Game1.content.LoadString("Strings\\UI:IslandGeodeMenu_Description_NotEnoughMoney");
				}
				else
				{
					descriptionText = Game1.content.LoadString("Strings\\UI:IslandGeodeMenu_Description");
				}
			}
		}
		...
		public override void update(GameTime time)
		{
			...
			int frame = lilac.currentFrame;
			lilac.animateOnce(time);
			if (lilac.currentFrame == 15 && frame != 15)
			{
				...
				if (geodeSpot.item != null)
				{
					...
					geodeDestructionAnimation = new TemporaryAnimatedSprite("TileSheets\\animations", new Microsoft.Xna.Framework.Rectangle(0, geodeDestructionYOffset, 64, 64), 100f, 8, 0, new Vector2(geodeSpot.bounds.X + 392 - 32, geodeSpot.bounds.Y + 192 - 32), flicker: false, flipped: false);
					if (geodeSpot.item != null && (int)geodeSpot.item.parentSheetIndex == 275)
					{
						geodeDestructionAnimation = new TemporaryAnimatedSprite
						{
							...
							sourceRect = new Microsoft.Xna.Framework.Rectangle(388, 123, 18, 21),
						...
						for (int i = 0; i < 6; i++)
						{
							fluffSprites.Add(new TemporaryAnimatedSprite("LooseSprites\\Cursors", new Microsoft.Xna.Framework.Rectangle(372, 1956, 10, 10), new Vector2(geodeSpot.bounds.X + 392 - 32 + Game1.random.Next(21), geodeSpot.bounds.Y + 192 - 16), flipped: false, 0.002f, new Color(255, 222, 198))
							{
								alphaFade = 0.02f,
							...
							{
								texture = Game1.temporaryContent.Load<Texture2D>("LooseSprites//temporary_sprites_1"),
								sourceRect = new Microsoft.Xna.Framework.Rectangle(499, 132, 5, 5),
					...
					if (!(this.geodeSpot.item.QualifiedItemId == "(O)275") && (!(geodeTreasure is Object) || !(geodeTreasure as Object).Type.Contains("Mineral")) && geodeTreasure is Object && (geodeTreasure as Object).Type.Contains("Arch") && !Game1.player.hasOrWillReceiveMail("artifactFound"))
					{
			...
			if (geodeDestructionAnimation != null && ((geodeDestructionAnimation.id != 777f && geodeDestructionAnimation.currentParentTileIndex < 7) || (geodeDestructionAnimation.id == 777f && geodeDestructionAnimation.currentParentTileIndex < 5)))
			{
				...
				else
				{
					...
					{
						sparkle = new TemporaryAnimatedSprite("TileSheets\\animations", new Microsoft.Xna.Framework.Rectangle(0, 640, 64, 64), 100f, 8, 0, new Vector2(geodeSpot.bounds.X + 392 - 32, geodeSpot.bounds.Y + 192 + yPositionOfGem - 32), flicker: false, flipped: false);
		...
		public override void gameWindowSizeChanged(Microsoft.Xna.Framework.Rectangle oldBounds, Microsoft.Xna.Framework.Rectangle newBounds)
		{
			...
			geodeSpot = new ClickableComponent(new Microsoft.Xna.Framework.Rectangle(xPositionOnScreen + IClickableMenu.spaceToClearSideBorder + IClickableMenu.borderWidth / 2, yPositionOnScreen + IClickableMenu.spaceToClearTopBorder + 4, 560, 308), "Anvil");
		...
		public override void draw(SpriteBatch b)
		{
			...
			b.Draw(Game1.mouseCursors, new Vector2(geodeSpot.bounds.X, geodeSpot.bounds.Y), new Microsoft.Xna.Framework.Rectangle(0, 512, 140, 78), Color.White, 0f, Vector2.Zero, 4f, SpriteEffects.None, 0.87f);
			if (geodeSpot.item != null)
			{
			...
			lilac.draw(b, new Vector2(geodeSpot.bounds.X + 384, geodeSpot.bounds.Y + 64), 0.877f);
			if (!hoverText.Equals(""))
			{

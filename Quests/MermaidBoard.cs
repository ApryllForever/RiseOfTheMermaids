...
{
	public class MermaidBoard : Billboard
	{
		...
		internal MermaidBoard(QuestData questData, string boardType = "") : base(dailyQuest: true)
		{
			this.questData = questData;
			this.boardType = boardType;
			timestampOpened = (int)Game1.currentGameTime.TotalGameTime.TotalMilliseconds;
			if (boardType == MermaidConstants.Z_RANGERBOARD)
			{
				this.dailyQuest = questData.dailyRangerQuest;
				this.acceptQuestButton.visible = !questData.acceptedDailyRangerQuest;
			}
			else
			{
				this.dailyQuest = questData.dailySapphireQuest;
				this.acceptQuestButton.visible = !questData.acceptedDailySapphireQuest;
			}
			if (dailyQuest != null)
			{
			...
			Texture2D billboardTexture;
			Log.Debug($"{boardType}, {MermaidConstants.Z_SAPPHIREBOARD}, {boardType.Equals(MermaidConstants.Z_SAPPHIREBOARD)}");
			if (boardType.Equals(MermaidConstants.Z_SAPPHIREBOARD))
			{
				billboardTexture = Game1.temporaryContent.Load<Texture2D>("LooseSprites\\SapphireBoard");

			}
			else if (boardType.Equals(MermaidConstants.Z_RANGERBOARD))
			{
				billboardTexture = Game1.temporaryContent.Load<Texture2D>("LooseSprites\\RangerBoard");
			}
			else
			{
		...
		public override void receiveLeftClick(int x, int y, bool playSound = true)
		{
			...
			if (this.acceptQuestButton.visible && this.acceptQuestButton.containsPoint(x, y) && this.dailyQuest != null)
			{
				...
				if (this.boardType == MermaidConstants.Z_RANGERBOARD)
				{
					this.questData.acceptedDailyRangerQuest = true;
				}
				else
				{
					this.questData.acceptedDailySapphireQuest = true;
				}

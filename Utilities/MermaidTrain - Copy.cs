...
namespace RestStopLocations
{
	...
	public class MermaidTrain : INetObject<NetFields>
	{
		...
		{
			return new Rectangle(-this.cars.Count * 128 * 4 + (int)this.position.X, 1310, this.cars.Count * 128 * 4, 128);
		}
		...
		public bool Update(GameTime time, GameLocation location)
		{
			...
			{
				Game1.player.currentLocation.temporarySprites.Add(new TemporaryAnimatedSprite(25, new Vector2(this.position.X - 170f, 96f), Color.White, 8, flipped: false, 100f, 0, 64, 1f, 128));
				this.smokeTimer = this.speed * 2000f;
			}
		...
		public void draw(SpriteBatch b)
		{
			...
			{
				this.cars[i].draw(b, new Vector2(this.position.X - (float)((i + 1) * 512), 1088f), this.wheelRotation);
			}
			if (this.whistleSteam != null)
			{
